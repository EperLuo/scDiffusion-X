...
def main():
    ...
    for model_path in multimodal_name_list:
        ...
        with open(f'{os.path.dirname(os.path.realpath(__file__))}/../../Autoencoder/configs/encoder/{args.encoder_config}.yaml', 'r') as file:
            yaml_content = file.read()
        autoencoder_args = yaml.safe_load(yaml_content)

        # Initialize encoder                'atac': 143810, 'rna': 19448
        autoencoder_args['encoder_kwargs']['rna']['norm_type']=args.norm_type
        autoencoder_args['encoder_kwargs']['atac']['norm_type']=args.norm_type
